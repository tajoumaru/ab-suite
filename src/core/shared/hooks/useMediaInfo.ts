...
async function fetchKitsuData(kitsuId: string): Promise<{ rating: number | null; votes: number | null }> {
  ...
  const kitsuData = await cachedApiCall(
    ...
    log("Failed to fetch Kitsu data", error);
    return null;
  });
...
async function fetchSimklData(
  ...
  if (existingSimklId) {
    ...
    const searchResults = await cachedApiCall(
      searchCacheKey,
      () =>
        apiRequest<SimklSearchResponse[]>({
          method: "GET",
      ...
      log("Failed to fetch SIMKL search data", error);
      return [];
    });
  ...
  const detailData = await cachedApiCall(
    detailCacheKey,
    () =>
      apiRequest<SimklDetailResponse>({
        method: "GET",
    ...
    log("Failed to fetch SIMKL detail data", error);
    return null;
  });
        ...
        if (malId) {
          try {
            ...
            apiData = await cachedApiCall(
              ...
              err("Failed to fetch anime API data", parseError);
              return null;
            });
          } catch (error) {
