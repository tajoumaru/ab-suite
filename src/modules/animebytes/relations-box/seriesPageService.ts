    ...
    GM_xmlhttpRequest({
      ...
      onload: (response) => {
        try {
          ...
          resolve({ entries, parentUrl });
        } catch (error) {
          reject(new Error(`Failed to parse series page: ${error instanceof Error ? error.message : "Unknown error"}`));
        }
      },
      onerror: () => {
        reject(new Error("Failed to fetch series page"));
      },
    });
  });
}
